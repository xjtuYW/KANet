...
def get_dataset_imagenet(args):
    import torchvision.datasets as datasets
    transform_ = transform.image_augment(state=args.state, dataset=args.dataset, image_size=args.image_size)
    if args.state == 'train':
...
def get_dataset(args, dataset_min_cls: int=0, dataset_max_cls: int=10000):
    transform_ = transform.image_augment(state=args.state, dataset=args.dataset, image_size=args.image_size)
    cwd = os.getcwd()
    if args.dataset == 'miniImageNet':
        ...
        data_path = os.path.join(cwd, 'datasets/MNIST')
    elif args.dataset == 'flowers':
        data_path = os.path.join(cwd, 'datasets/flowers')
    elif args.dataset == 'food101':
        data_path = os.path.join(cwd, 'datasets/food101')
    elif args.dataset == 'car196':
        data_path = os.path.join(cwd, 'datasets/car196')
    elif args.dataset == 'aircraft102':
        data_path = os.path.join(cwd, 'datasets/aircraft102')
    elif args.dataset == 'cifar10':
        data_path = os.path.join(cwd, 'datasets/cifar10')
    else:
...
def get_sampler(dataset, args, sample_info):
    ...
    elif sampler_type == 'ClsSampler': # 
        sampler_ = sampler.ClsSampler(dataset.labels)
...
def get_dataloader(args, dataset_min_cls: int=0, dataset_max_cls: int=10000, sample_info=None):
    if args.dataset == 'ImageNet':
        dataset = get_dataset_imagenet(args)
    else:
        dataset = get_dataset(args, dataset_min_cls=dataset_min_cls, dataset_max_cls=dataset_max_cls)
    ...
    if args.world_size > 1 and args.sampler == 'std':
        ...
        if args.sampler == 'std':
            batch_size = args.batch_size if args.state == 'train' else args.batch_size_test
            shuffle    = True if args.state == 'train' else False
            loader = torch.utils.data.DataLoader(dataset, batch_size, shuffle=shuffle, num_workers=args.workers, pin_memory=True)
        else:
